...
PUBLISH_COMPONENT = ROOT / "scripts" / "components" / "publish_outputs.py"


def _load_publish_component():
    spec = importlib.util.spec_from_file_location("publish_outputs", PUBLISH_COMPONENT)
...
def test_publish_outputs_validates_auth_monitoring_contract_before_upload(tmp_path):
...
def test_publish_outputs_rejects_incomplete_auth_monitoring_contract(tmp_path):
    module = _load_publish_component()
    artifacts = [
        path
...
def test_publish_outputs_uses_managed_identity_client_id(monkeypatch):
    module = _load_publish_component()
    captured = {}
...
def test_publish_outputs_resolves_config_path_relative_to_component_source():
    module = _load_publish_component()
    resolved = module._resolve_config_path(
        ROOT / "tmp" / "downloaded-run",
        Path("configs/drift_thresholds.json"),
    )

    assert resolved == (ROOT / "scripts" / "components" / "configs" / "drift_thresholds.json").resolve()
