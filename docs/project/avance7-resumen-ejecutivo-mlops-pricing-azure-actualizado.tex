...
Este resumen integra los avances previos en una sola narrativa --el problema de negocio, los hallazgos del análisis exploratorio y de preparación, el modelo elegido con sus especificaciones, y las recomendaciones de implementación-- bajo un mismo hilo conductor: reentrenar por evidencia, no por calendario.
...
El beneficio principal es control del ciclo de vida del modelo. El ahorro de GPU se vuelve relevante cuando el proceso actual de negocio reentrena el modelo mensualmente en una NVIDIA A10. En ese caso, el monitoreo de drift convierte el gasto de GPU de un costo fijo por calendario a una decisión condicionada por evidencia. El ahorro más sólido combina cómputo evitado, menor carga de validación y menor riesgo de cambiar un modelo estable sin necesidad.
...
Ahorro de cómputo & Menos uso de GPU A10 cuando no hay señal de retrain. & Evitar retraining mensual cuando drift no justifica nueva versión. & Alta \\
Escalabilidad & El monitoreo puede automatizarse antes del retraining. & Gating previo a entrenamiento. & Alta \\
\bottomrule
\end{tabularx}

\subsection{Impacto de evitar retraining mensual en GPU A10}

El dato operativo de referencia es que, en la empresa, el modelo se entrena cada mes en una GPU NVIDIA A10. Para estimar el orden de magnitud en cloud, la Azure Retail Prices API reporta \texttt{Standard\_NV36ads\_A10\_v5} en \texttt{eastus2} a USD 3.20 por hora. En contraste, el monitoreo actual corre en \texttt{cpu-cluster} con \texttt{STANDARD\_DS2\_V2}, aproximadamente USD 0.114 por hora. Por eso, una corrida de drift cuesta centavos, mientras que un retraining A10 puede costar decenas o cientos de dólares según duración.

\begin{tabularx}{\linewidth}{L{0.24\linewidth}L{0.20\linewidth}L{0.22\linewidth}Y}
\toprule
\textbf{Duración del retraining mensual} & \textbf{Costo por retrain A10} & \textbf{Costo anual por calendario} & \textbf{Lectura ejecutiva} \\
\midrule
4 horas & USD 12.80 & USD 153.60 & Ahorro directo moderado; sigue evitando validación innecesaria. \\
8 horas & USD 25.60 & USD 307.20 & Ya hay ahorro visible si varios meses no requieren retraining. \\
24 horas & USD 76.80 & USD 921.60 & El gating por drift puede evitar casi mil dólares anuales por modelo. \\
48 horas & USD 153.60 & USD 1,843.20 & El valor económico se vuelve material, además de liberar capacidad GPU. \\
\bottomrule
\end{tabularx}

El costo por calendario es el límite superior del gasto, no el ahorro. El ahorro real depende de cuántos de esos doce meses presentan evidencia de drift suficiente para reentrenar. Si el drift solo justifica $n$ retrainings en el año, se evitan los $12 - n$ restantes; el ahorro de cómputo es $(12 - n)$ veces el costo de un retraining, y el costo del monitoreo es prácticamente ruido frente a ese término.

\begin{tabularx}{\linewidth}{L{0.22\linewidth}L{0.18\linewidth}L{0.18\linewidth}Y}
\toprule
\textbf{Duración del retraining} & \textbf{2 retrains/año (evita 10)} & \textbf{4 retrains/año (evita 8)} & \textbf{6 retrains/año (evita 6)} \\
\midrule
4 horas & USD 128.00 & USD 102.40 & USD 76.80 \\
8 horas & USD 256.00 & USD 204.80 & USD 153.60 \\
24 horas & USD 768.00 & USD 614.40 & USD 460.80 \\
48 horas & USD 1,536.00 & USD 1,228.80 & USD 921.60 \\
\bottomrule
\end{tabularx}

La lectura es directa: con retrainings largos y drift poco frecuente, el ahorro de cómputo va de cientos a más de mil dólares anuales por modelo. En el peor caso, cuando el mercado cambia los doce meses, el ahorro de cómputo es cero, pero el modelo se reentrena con evidencia y traza auditable, no a ciegas; nunca se gasta de más respecto al calendario actual.

El otro lado de la comparación es que monitorear es casi gratis. Una corrida de drift sobre las features agregadas corre en \texttt{cpu-cluster} en minutos; aun monitoreando a diario, el costo anual de monitoreo (aproximadamente USD 10 a USD 40 según el tiempo de arranque del clúster) es menor que un solo retraining A10 de 24 horas. Esto desacopla la frecuencia de detección de la frecuencia de retraining: se puede vigilar el modelo todos los días y reentrenarlo solo cuando hay evidencia, en vez de pagar GPU una vez al mes y aun así detectar el drift tarde.

El cómputo evitado es solo la capa visible del ahorro. Cada retraining por calendario también consume horas de validación de la nueva versión y agrega riesgo de cambiar un modelo estable sin necesidad; el gating por drift elimina esos dos costos en los meses sin evidencia y libera capacidad GPU para otros trabajos. Por eso el beneficio económico total es mayor que la sola diferencia de factura cloud.

Si la NVIDIA A10 es on-premise o ya está asignada por contrato interno, el ahorro puede no aparecer como reducción directa de factura cloud. Aun así, se conserva como costo de oportunidad: horas GPU liberadas, menor retrabajo de validación y menor riesgo de introducir una versión nueva sin evidencia.

\statusbox{\textbf{Lectura económica:} basta con que el drift evite un (1) retraining al año para que todo el monitoreo, incluso diario, quede pagado varias veces; a partir de ahí el ahorro escala con la duración del retraining y con cada mes que no requiere una versión nueva. El supuesto es que el drift material ocurre menos de doce veces al año; un backstop de antigüedad máxima del modelo acota el riesgo de conservar una versión vieja.}

\subsection{Beneficios tangibles e intangibles}

\textbf{Tangibles (cuantificables).}
\begin{itemize}
  \item Cómputo GPU evitado: entre USD 76 y USD 1,536 al año por modelo, según la duración del retraining y la frecuencia real de drift (tabla anterior).
  \item Horas de validación evitadas: si cada retraining por calendario consume del orden de 4 a 8 horas de validación de la nueva versión en el proceso de negocio, evitar 8 de 12 al año libera aproximadamente 32 a 64 horas por modelo al año.
  \item Capacidad GPU liberada: cada mes sin retraining innecesario devuelve horas de A10 a otros trabajos.
  \item Costo de monitoreo marginal: vigilar a diario cuesta menos que un solo retraining A10 de 24 horas.
\end{itemize}

\textbf{Intangibles (cualitativos).}
\begin{itemize}
  \item Gobernanza con evidencia: reentrenar deja de depender del calendario o la intuición. Ejemplo: con seis meses de datos AUTH el drift no justificó retraining completo, y esa decisión quedó respaldada por métricas y no por rutina.
  \item Menor riesgo de cambio: no se introduce una versión nueva sobre un modelo estable sin señal, evitando inestabilidad de precios frente a negocio.
  \item Auditabilidad: cada corrida deja run logs, snapshots y contratos que permiten reconstruir por qué se tomó una decisión.
  \item Detección más temprana: como el monitoreo es barato, puede correr más seguido que el calendario y detectar drift a mitad de ciclo.
  \item Confianza de negocio: las señales Yellow/Red priorizadas por revenue habilitan conversaciones accionables en lugar de alarmas genéricas.
\end{itemize}
...
Este es un proyecto de MLOps puro: el equipo no entrena el modelo --eso pertenece al proceso de Data Science y de negocio--, por lo que no hay costo de cómputo de entrenamiento del lado de la plataforma. Como no se manejan facturas separadas ni horas registradas, el costo se estima por características de la arquitectura: el inventario de recursos según su SKU y patrón de uso, valuado con la Azure Retail Prices API (\texttt{eastus2}, Linux). El costo grande --la GPU A10 de retraining-- es del proceso de negocio, y es justo el que esta solución vuelve condicional.

\begin{tabularx}{\linewidth}{L{0.19\linewidth}L{0.31\linewidth}L{0.18\linewidth}Y}
\toprule
\textbf{Fase CRISP-ML} & \textbf{Recurso / característica} & \textbf{Costo (orden de magnitud)} & \textbf{Naturaleza} \\
\midrule
Business/Data Understanding & Sin infraestructura; análisis y EDA sobre datos masked. & USD 0 & No recurrente. \\
Data Preparation & Storage \texttt{Standard\_LRS} para masked/curated/baseline (pocos GB). & Centavos al mes & Recurrente bajo. \\
Modeling & Modelo HB-SVI externo/pre-entrenado; el equipo solo lo integra, sin GPU del lado de la plataforma. & USD 0 en plataforma & N/A para la plataforma. \\
Evaluation & Baseline, drift y validaciones en \texttt{cpu-cluster} \texttt{DS2\_v2} (USD 0.114/h), que escala a cero. & Aprox. USD 0.06-0.11 por corrida & Recurrente por corrida. \\
Deployment & AML Workspace (sin cargo base), batch endpoint bajo demanda, Key Vault, IaC; eventual Container Registry. & Aprox. USD 0-5/mes & Recurrente bajo. \\
Monitoring & Observabilidad (Log Analytics / App Insights) por ingestión más corridas de drift. & Aprox. USD 0-5/mes & Recurrente bajo. \\
Retraining (negocio) & GPU \texttt{NV36ads\_A10\_v5} (USD 3.20/h); condicionada por drift, no por calendario. & USD 12.80-153.60 por retrain & Variable, evitable. \\
\bottomrule
\end{tabularx}

En conjunto, la huella cloud del MVP es del orden de USD 5 a USD 25 al mes --storage, observabilidad y compute bajo demanda, con el \texttt{cpu-cluster} en cero entre corridas--, lo que acumula decenas de dólares a la fecha, no miles. El costo material no vive en la plataforma sino en el retraining GPU del proceso de negocio: ahí es donde el gating por drift convierte un gasto fijo por calendario en uno condicionado por evidencia (ver las tablas A10 y de ahorro).
...
Modelo (técnico) & Umbrales de drift mal calibrados que generen falsos negativos o falsos positivos. & Conservar un modelo viejo o reentrenar sin necesidad. & Calibrar umbrales con negocio y aplicar un backstop de antigüedad máxima del modelo. \\
Negocio/valor & Comunicar el ahorro de GPU como el beneficio principal. & Subvaluar la gobernanza del ciclo de vida del modelo. & Posicionar el valor como decisión por evidencia; el ahorro es secundario. \\
\bottomrule
\end{longtable}
...
\begin{tabularx}{\linewidth}{L{0.32\linewidth}L{0.13\linewidth}Y}
\toprule
\textbf{Recomendación} & \textbf{Prioridad} & \textbf{Justificación} \\
\midrule
Mantener el flujo en \texttt{staging}/\texttt{validation} sin promover a producción todavía. & Alta & El pipeline versionado aún debe estabilizarse y faltan capas de model health y gobierno. \\
Conservar la política de retraining por evidencia y añadir un backstop de antigüedad máxima del modelo. & Alta & Acota el riesgo de staleness por falsos negativos del monitoreo. \\
Formalizar con negocio los thresholds de drift (PSI, KS, coverage). & Alta & Convierte los umbrales técnicos en criterios acordados y auditables. \\
Completar model health: demanda estimada por S-curve vs real y salud de elasticidades. & Media-alta & Hoy el monitoreo es operacional/pre-model; falta la degradación predictiva. \\
Conectar auditoría SQL productiva solo si aparece un requisito de consulta. & Media & Evita complejidad innecesaria; hoy basta la evidencia versionada en Storage. \\
Mantener el Azure ML batch endpoint como única ruta de orquestación. & Media & Evita doble ownership con ADF o Functions. \\
\bottomrule
\end{tabularx}
