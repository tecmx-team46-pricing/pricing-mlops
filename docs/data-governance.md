...
El workflow manual de Azure no lee ni escribe `raw-unmasked`. El dataset compartido inicial debe vivir en `raw-masked/samples/sample_pricing_v1.csv`; el compute target lo lee con managed identity y escribe outputs versionados por `environment`, `compute`, `owner`, `run_date` y `run_id`.
